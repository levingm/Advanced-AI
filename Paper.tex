...
\subsection{Regularized Linear Regression: The Gold Standard for Stability}
The Ridge-regularized Linear Model achieved the highest overall accuracy ($\text{MAE} = 9.36$~Bn.~\texteuro), outperforming the Last-change benchmark by $20.5\%$ and remaining the **only model to achieve a positive mean $R^2$ ($0.178$)**. 
...
For bank Asset-Liability Management (ALM) and treasury operations, predictability, computational speed, and interpretability are critical. Our benchmark demonstrates that complex deep learning models do not automatically translate into superior deposit forecasts when future macro-covariates are uncertain. **Regularized Linear Regression provides the most robust, interpretable, and accurate framework for 100-day NMD volume forecasting.**
...
\begin{enumerate}
    \item \textbf{Model Complexity vs. Accuracy (RQ1 \& RQ2):} Greater model complexity does not guarantee higher forecast accuracy. The Ridge-regularized Linear Model achieved the best overall performance ($\text{MAE} = 9.36$~Bn.~\texteuro, $R^2 = 0.178$), outperforming complex neural networks (FFN MAE $11.85$~Bn.~\texteuro) and boosted trees (XGBoost MAE $12.71$~Bn.~\texteuro).
