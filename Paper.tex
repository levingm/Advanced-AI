...
Detailed non-maturity deposits (NMDs) data are generally not publicly available because it reveals sensitive bank information. For this study, we utilize public end-of-period levels of overnight deposits held by private households data, published by the Deutsche Bundesbank \citep{bundesbank_data} as a proxy for aggregate NMD volumes. Household overnight deposits provide an accurate publicly observable proxy since they are a subset of NMDs and account for a substantial share of the banks' total deposit funding \citep{bundesbank2024financing}. Therefore, the aggregate volume of household overnight deposits are likely to respond to several of the same economic drivers. The dataset covers the period January 2003 to September 2025 and the series is measured in billions of euros. 
...
% - Preprocessing steps
\subsection{Data Allignment and Transformation: Interpolation Methods}
...
\label{subsec:forecasting_problem}

Let \(y_t\) denote the volume of German household overnight deposits at time
\(t\), and let \(\mathbf{x}_t\) denote the vector of macroeconomic and
financial covariates. The objective is to forecast deposit volumes over a
100-day horizon using the information available during the preceding 30 days.

Because the deposit-volume series exhibits a strong long-term trend, the models
are trained to predict its first difference,
\begin{equation}
    \Delta y_t = y_t-y_{t-1}.
    \label{eq:first_difference}
\end{equation}

At each forecast origin \(t\), the forecasting models use a lookback window of
length \(L=30\). The information set therefore contains the historical deposit
...
The study therefore applies a direct multi-output forecasting strategy. Rather
than generating forecasts recursively one day at a time, each model estimates
all 100 future changes simultaneously. The predicted changes are subsequently
...
This reconstruction permits the models to be trained on the less strongly
trending differenced series while their performance is evaluated in terms of
the economically interpretable deposit-volume level.\\
\indent At each forecast origin, only observations available up to that date are used
for model estimation and feature scaling. Realized future values of the
economic covariates are not supplied to the models. Calendar variables whose
future values are known in advance, such as the day of the week or month-end
indicator, may be included where required by the model architecture. The
model-specific treatment of future covariates is describe later. 
...
related variable groups, individual interest-rate variables, and ablations of
the complete information set. The naive benchmarks use no covariates.
...
\end{equation}
The models are re-estimated at every origin as newly observed data become
available. The evaluation uses four origins: 1 January 2009, 1 January 2013,
1 March 2019, and 1 June 2025.

All preprocessing parameters, including feature scaling, are estimated using
the corresponding training sample only. Realized economic covariates beyond
the forecast origin are not supplied to the models. For the Temporal Fusion
Transformer, future calendar variables are known in advance, whereas economic
covariates are held at their latest observed values throughout the forecast
horizon.
