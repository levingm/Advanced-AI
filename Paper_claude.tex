...
overnight deposits account for the largest single component of retail funding
\citep{bundesbank2024deposits}, making their predictable management critical for bank liquidity management,
balance-sheet planning, and regulatory compliance \citep{bcbs2016irrbb}. However, because depositor behavior is driven by
stochastic cash-flow needs \citep{marena2022nonmaturing}, the risk management of NMDs has long been understudied despite to its importance \citep{kalkbrener2004}, with no consensus on the optimal modeling approach.
...
arbitrage-free framework. Another way is to translate volume dynamics into
quantitative frameworks for asset--liability management
...
model superiority from window-specific fit \citep{hewamalage2023}.


\paragraph{Complexity in Forecasting}
Other literature on forecasting already showed that simple statistical and hybrid models systematically beat pure ML methods over more complex models \citep{makridakis2020m4}, this holds even in large-scale retail settings \citep{makridakis2022m5}.
...
disrupts temporal ordering. \citet{hewamalage2023} show that fixed-origin evaluation on a single 
test window is insufficient, leading to false conclusions, 
recommending rolling-origin protocols as more robust.
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{l X c c}
        \toprule
        Reference & Modelling approach & Multiple origins & Matched tuning \\
        \midrule
        \citet{marena2022nonmaturing} & Lévy--OU stochastic model & \ding{55} & \ding{55} \\
        \citet{winkler2025}       & Lévy OU process vs.\ LSTM & \ding{55} & \ding{55} \\
        ...
        \midrule
        \textbf{This paper} & Naïve, Ridge, XGBoost, FFN, TFT & \ding{51} & \ding{51} \\
        \bottomrule
    \end{tabularx}
\end{table}
