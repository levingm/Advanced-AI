...
\author[1]{Maté Szabo \small Matr.-Nr.: XXXXXXX}
...
\begin{abstract}
    ...
    the optimal modeling approach. This study compares naive
    ...
    distinct interest-rate regimes between 2009 and 2025. The
    regularized linear model achieved the lowest mean forecast error
    (MAE $= 9.36$~Bn.~\texteuro) and was the only model with a
    positive mean $R^2$ ($0.178$), outperforming the na\"ive
    random-walk benchmark (MAE $= 11.78$~Bn.~\texteuro) by
    $20.5\%$, while more complex non-linear architectures did not
    consistently improve on this simple benchmark. These results
    indicate that increased model complexity does not necessarily
    translate into higher forecasting accuracy for this task, with
    direct implications for how banks might select forecasting tools
...
provides related literature on NMD modeling and forecasting, and
positions this study within this research. Section~3 presents the
data and preprocessing methodology, including variable definitions,
interpolation, and target transformation. Section~4 describes the
forecasting framework, the models under comparison, and the
rolling-origin evaluation design. Section~5 reports and compares
the forecasting results. Section~6 discusses these findings and
their managerial implications. Finally, Section~7 concludes the
...
\begin{table}[htbp]
    \centering
    \caption{Positioning of this paper relative to related NMD research.}
    \label{tab:related_work}
    \begin{tabularx}{\textwidth}{l X X}
        \toprule
        Reference & Modeling Approach & Rolling-Origin Evaluation \\
        \midrule
        \citet{winkler2025} & L\'evy-driven OU process vs.\ LSTM & \ding{55} \\
        \midrule
        \textbf{This paper} & Naive, Ridge, XGBoost, FFN, TFT comparison & \ding{51} \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\textbf{Deposit Rate:} refers to the household deposit interest
rate, sourced from the Deutsche Bundesbank \citep{bundesbank_data}.
The more interest households receive on deposits, the more
attractive it may be to keep their money in overnight deposit
accounts rather than seeking alternative investments.
...
The models are re-estimated at every origin as newly observed data becomes
available. The evaluation uses four origins: 1 January 2009, 1 January 2013,
1 March 2019, and 1 June 2025. These dates were selected to represent
distinct interest-rate and macroeconomic regimes -- the 2008/2009 financial
crisis, the subsequent low-interest-rate period, the pre-COVID environment,
and the post-2022 high-interest-rate tightening cycle -- rather than a dense
monthly rolling scheme, in order to explicitly test model robustness across
qualitatively different economic conditions.
...
for pairwise significance of forecast accuracy differences was implemented
but not applied to the main results, as the four rolling origins used here
provide insufficient statistical power for a reliable significance test;
model comparisons are therefore reported descriptively rather than as
...
The empirical findings from our rolling-origin benchmark provide crucial insights into how distinct algorithmic architectures handle highly autocorrelated, noisy, and regime-shifting financial time series over a $100$-day forecasting horizon. Contrary to common expectations in modern machine learning literature, our evaluation across four distinct macroeconomic interest rate regimes ($2009$--$2025$) reveals that \textbf{simpler, regularized linear models outperform complex non-linear architectures}.

This section critically evaluates the performance, structural strengths, and architectural constraints of all six benchmarked models based on the empirical results presented in Table~\ref{tab:overall_results}.

\subsection{Ridge Regression: The Gold Standard for Stability}
Ridge achieved the highest overall accuracy ($\text{MAE} = 9.36$~Bn.~\texteuro), outperforming Random Walk by $20.5\%$ and remaining the **only model to achieve a positive mean $R^2$ ($0.178$)**. 
...
\begin{equation}
    \widehat{y}_{t_0+h} = y_{t_0} + \sum_{j=1}^{h} \widehat{\Delta y}_{t_0+j}
\end{equation}
The Ridge penalty shrinks non-essential feature weights toward zero, effectively filtering out high-frequency interbank noise. Consequently, when supplied with the full feature space (\texttt{All covariates}), Linear Regression maintained superior stability across major interest rate pivots.

\subsection{Temporal Fusion Transformer (TFT): Architectural Overhead vs. Input Constraints}
The Temporal Fusion Transformer ranked second overall ($\text{MAE} = 10.78$~Bn.~\texteuro). Although TFT is designed specifically for multi-horizon time series forecasting via self-attention mechanisms, its full performance potential was constrained by the strict ex-post evaluation setup.
...
\subsection{Feedforward Neural Networks (FFN) and XGBoost: Overfitting and Extrapolation Traps}
Neither FFN ($\text{MAE} = 11.85$~Bn.~\texteuro) nor XGBoost ($\text{MAE} = 12.71$~Bn.~\texteuro) managed to consistently outperform the simple Random Walk benchmark ($\text{MAE} = 11.78$~Bn.~\texteuro) on average:
\begin{itemize}
    \item \textbf{FFN Consistency vs. Precision:} Interestingly, the FFN exhibited the lowest standard deviation across forecast origins ($\pm 6.90$~Bn.~\texteuro), indicating high cross-regime consistency. However, its average MAE remained slightly worse than the simple benchmark, showing that unregularized dense layers tend to fit noise in daily volume differences.
    \item \textbf{XGBoost Extrapolation Limitation:} Decision trees partition the feature space into orthogonal hyper-rectangles and cannot extrapolate continuous trends beyond the bounded range of historical training data. When deposit volumes reached new record levels (such as during the 2022--2025 rate tightening cycle), XGBoost capped predictions at historical leaf thresholds, causing systematic underestimation.
\end{itemize}

\subsection{Understanding the Out-of-Sample $R^2$ Metric in Time Series}
As observed in Table~\ref{tab:overall_results}, several models yielded negative $R^2$ values. In multi-step time series forecasting, $R^2$ evaluates predictions against the localized mean of the test evaluation window ($\overline{y}_k$). Because $\overline{y}_k$ is calculated *post-hoc* using future realizations unavailable at the forecast origin, it represents an unrealistically strong benchmark. A negative $R^2$ indicates that a model's multi-step trajectory diverged from this post-hoc mean, rather than indicating zero predictive utility. Therefore, Level MAE serves as our primary metric for practical model ranking.

%\begin{figure}[htbp]
%    \centering
%    \includegraphics[width=0.85\textwidth]{figures/error_accumulation_horizon.png}
%    \caption{Cumulative forecast error accumulation across all evaluated model architectures over the $100$-day forecasting horizon.}
%    \label{fig:error_accumulation}
%\end{figure}

\subsection{Managerial Implications and Practical Takeaways}
For bank Asset-Liability Management (ALM) and treasury operations, predictability, computational speed, and interpretability are critical. Our benchmark demonstrates that complex deep learning models do not automatically translate into superior deposit forecasts when future macro-covariates are uncertain. \textbf{Ridge provides the most robust, interpretable, and accurate framework for 100-day NMD volume forecasting.}

% ---------------------------------------------------------------------------
% 6. Conclusion and Outlook
% ---------------------------------------------------------------------------
% Guidance (p.26), three-part structure:f
...
\begin{enumerate}
    \item \textbf{Model Complexity vs. Accuracy (RQ1 \& RQ2):} Greater model complexity does not guarantee higher forecast accuracy. Ridge achieved the best overall performance ($\text{MAE} = 9.36$~Bn.~\texteuro, $R^2 = 0.178$), outperforming FFN (MAE $11.85$~Bn.~\texteuro) and XGBoost (MAE $12.71$~Bn.~\texteuro).
    \item \textbf{Covariate Selection (RQ3):} The predictive value of covariates depends heavily on model architecture. Linear models benefit from broad, multi-variable feature sets (\texttt{All covariates}) where regularized weights filter noise, whereas non-linear models performed best when restricted to core macroeconomic indicators (Inflation and Geopolitical Risk).
    \item \textbf{Practical ALM Utility:} For bank liquidity buffer management (e.g., LCR requirements), simple regularized models offer minimal computational overhead, complete transparency, and robust protection against regime shifts.
\end{enumerate}

% --- Part 3: Outlook / future work ---
\paragraph{Outlook and Future Work.}
Future research could extend this comparative framework by incorporating scenario-based path projections for dynamic covariates rather than holding them constant over the forecast horizon. Additionally, combining linear baseline trends with non-linear residual models (hybrid architectures) may better capture sudden systemic liquidity shocks while preserving long-term trend stability.
